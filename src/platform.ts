...
async function augustValidateCode(code: AugustSession, session: AugustSession, log: Logger) {
  const payload = {
    code,
  };
...
async function augustGetHouses(session: AugustSession, log: Logger): Promise<AugustHome[]> {
  const options = {
    hostname: 'api-production.august.com',
    port: 443,
